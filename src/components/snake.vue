...
<div id="container" :style="containerStyle" class="container">
  <div v-show="!isStart" class="mask">
    <div v-show="isSelectDiffcult && !isShowScore" :style="diffcultChoose" class="diffcultChoose">
      <a :style="btnStyle" @click="chooseDiff('easy')" href="javascript:;">容易</a>
      <a :style="btnStyle" @click="chooseDiff('normal')" href="javascript:;">正常</a>
      <a :style="btnStyle" @click="chooseDiff('hard')" href="javascript:;">困难</a>
    </div>
    <div v-show="!isSelectDiffcult && !isShowScore" :style="menuStyle" class="btn-group">
      <h1 :style="titleStyle">贪吃蛇大作战</h1>
      <p :style="pStyle">困难等级 : {{diffcult==200?"EASY":diffcult==50?'困难':'正常'}}</p>
      <a :style="btnStyle" @click="start()" class="primary" href="javascript:;">开始</a>
      <a :style="btnStyle" @click="isSelectDiffcult = true" href="javascript:;">困难等级</a>
      <a :style="btnStyle" @click="showScore()" href="javascript:;">分数</a>
    </div>
    <div v-show="!isSelectDiffcult && isShowScore" :style="menuStyle" class="scoreWell">
      <h1 :style="titleStyle">最高得分</h1>
      <p :style="pStyle">最高分 : {{highScore}}<br><br>当前等级 : {{highScoreDiffcult}}</p>
      <a :style="btnStyle" @click="isShowScore = false;" href="javascript:;" class="primary">菜单</a>
    <a :style="btnStyle" @click="clearScore()" href="javascript:;">清除分数</a>
    </div>
  </div>
  <div v-show="isFailedFlag" class="failed mask">
    <h1 :style="titleStyle">您输了！</h1>
    <p :style="pStyle">分数 : {{score}}</p>
    <p style="margin-bottom: 30px;font-size: 14px;font-style: italic;font-weight: 300;color:#666;">:-D 您吃了 {{snakeArr.length-4}} 只虫子</p>
    <a @click="start()" :style="btnStyle" class="primary" href="javascript:;">重新开始</a>
    <a @click="isFailedFlag = false;isStart = false;" :style="btnStyle" href="javascript:;">菜单</a>
  </div>
  <div v-bind:style="[isSnake($key) && isSnakeHead($key) && isStart?{backgroundColor:'#888'}:'',itemStyle]" :class="[isSnake($key) && isStart?'snake':'',food==$key && isStart?'food':'']" v-for="(item,$key) in 576" class="item"></div>
</div>
...
data(){
  ...
  methods:{
    ...
    initStyle:function(){
			var clientW = document.body.clientWidth*0.8;
    ...
    GameFailed:function(){
      ...
      if(localStorage.getItem("score")){
        if(JSON.parse(localStorage.getItem("score")).score<this.score){
          localStorage.setItem("score",JSON.stringify({diffcult:this.diffcult==120?'正常'
            :this.diffcult==50?'困难'
            :'容易',
            score:this.score}));
        }
